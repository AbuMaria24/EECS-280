...
// ---------- DO NOT CHANGE ANYTHING IN THIS FILE ABOVE THIS LINE ----------
  ...
  static bool check_sorting_invariant_range(const Node *node,
                                          const T *min_value,
                                          const T *max_value,
                                          Compare less) {
    if (node == nullptr) {
    ...
    if (min_value != nullptr &&
        !less(*min_value, node->datum)) {
        return false;
    }

    if (max_value != nullptr &&
        !less(node->datum, *max_value)) {
        return false;
    }

    return check_sorting_invariant_range(node->left,
                                         min_value,
                                         &node->datum,
                                         less) &&
           check_sorting_invariant_range(node->right,
                                         &node->datum,
                                         max_value,
                                         less);
}
  ...
  static bool check_sorting_invariant_impl(const Node *node, Compare less) {
    return check_sorting_invariant_range(node, nullptr, nullptr, less);
  }
